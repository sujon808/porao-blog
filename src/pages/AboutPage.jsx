      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-400 text-lg max-w-2xl mx-auto leading-relaxed">
            We are a small team of writers, thinkers, and makers who believe that good ideas deserve beautiful writing.
          </p>
        </div>
        ...
        <div className="glass-card rounded-3xl p-10 md:p-14 mb-16 text-center relative overflow-hidden">
          ...
          <p className="text-slate-400 text-lg leading-relaxed max-w-2xl mx-auto">
            Porao Blog exists to slow things down — to explore ideas thoroughly, write with care, and give readers something worth their attention. We cover technology, design, science, travel, food, and lifestyle through the lens of curiosity and craft.
          </p>
        </div>

        <div className="grid grid-cols-3 gap-4 mb-16">
          {[
            { icon: Icon.FileText, label: 'Articles Published', value: posts.length + '+' },
            { icon: Icon.Users, label: 'Monthly Readers', value: '8K+' },
            { icon: Icon.Mail, label: 'Newsletter Subscribers', value: '5K+' },
          ].map(({ icon: Ic, label, value }) => (
            <div key={label} className="glass-card rounded-2xl p-6 text-center">
              <Ic size={22} className="text-sky-400 mx-auto mb-3" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {authors.map((author) => (
              <div key={author.id} className="glass-card rounded-2xl p-6 text-center hover:border-sky-500/30 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-sky-500/5">
                <img src={author.avatar} alt={author.name} className="w-20 h-20 rounded-2xl mx-auto mb-4 bg-slate-800 ring-4 ring-sky-500/10" />
                ...
                <a href={`https://twitter.com/${author.twitter.replace('@', '')}`} target="_blank" rel="noreferrer"
                  className="inline-flex items-center gap-1.5 mt-4 text-xs text-slate-400 hover:text-sky-400 transition-colors">
                  <Icon.Twitter size={12} /> {author.twitter}
                </a>
              </div>
            ))}
          </div>
        ...
        <div className="text-center glass-card rounded-3xl p-10">
          ...
          <Link to="/contact" className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl bg-sky-500 hover:bg-sky-400 text-white font-semibold transition-colors shadow-lg shadow-sky-500/20">
            Get in touch
          </Link>
        </div>
      </div>
