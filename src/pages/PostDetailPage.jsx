    ...
    <main className="hero-gradient min-h-screen pt-24 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link to="/blog" className="inline-flex items-center gap-2 text-slate-400 hover:text-sky-400 text-sm font-medium mb-8 transition-colors group">
          <Icon.ArrowLeft size={16} className="group-hover:-translate-x-0.5 transition-transform" /> Back to Blog
        </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
          <article className="lg:col-span-3">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
              ...
              <span className="flex items-center gap-1.5 text-slate-500 text-sm">
                <Icon.Clock size={13} /> {post.readTime}
              </span>
            </div>

            <h1 className="font-serif text-3xl md:text-4xl lg:text-5xl font-bold text-slate-100 leading-tight mb-6">{post.title}</h1>
            ...
            <div className="flex items-center gap-4 mb-10 pb-8 border-b border-slate-800/60">
              ...
              <div className="ml-auto flex items-center gap-2">
                <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}`} target="_blank" rel="noreferrer"
                  className="p-2 rounded-lg text-slate-400 hover:text-sky-400 hover:bg-sky-500/10 transition-colors border border-slate-700/40">
            ...
            <div className="mt-10 pt-8 border-t border-slate-800/60">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {post.tags.map((tag) => (
                  <span key={tag} className="px-3 py-1 rounded-full bg-slate-800/60 border border-slate-700/40 text-slate-400 text-xs font-medium hover:text-sky-400 hover:border-sky-500/30 cursor-pointer transition-colors">#{tag}</span>
                ))}
              </div>
            </div>
          ...
          <aside className="flex flex-col gap-6">
            ...
            {related.length > 0 && (
              <div className="glass-card rounded-2xl p-6">
                ...
                <div className="flex flex-col gap-3">{related.map((rp) => <PostCard key={rp.id} post={rp} />)}</div>
              </div>
            )}
          </aside>
        </div>
        ...
        {related.length > 0 && (
          <section className="mt-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {related.map((rp) => <PostCard key={rp.id} post={rp} featured />)}
            </div>
          </section>
        )}
