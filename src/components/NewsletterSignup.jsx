  ...
  if (compact) {
    return (
      <div className="glass-card rounded-2xl p-6">
        ...
        {submitted ? (
          <div className="flex items-center gap-2 text-emerald-400 text-sm"><Icon.CheckCircle size={16} /> You're subscribed!</div>
        ) : (
          <form onSubmit={handleSubmit} className="flex flex-col gap-2">
            ...
            <button type="submit" className="w-full px-4 py-2.5 rounded-xl bg-sky-500 hover:bg-sky-400 text-white text-sm font-semibold transition-colors shadow-lg shadow-sky-500/20">Subscribe</button>
          </form>
        )}
      </div>
    );
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl border border-sky-500/20 bg-gradient-to-br from-sky-500/10 via-slate-900 to-violet-500/10 p-10 md:p-16 text-center">
      ...
      <div className="relative">
        ...
        <h2 className="font-serif text-3xl md:text-4xl font-bold text-slate-100 mb-3">Never miss a story</h2>
        <p className="text-slate-400 max-w-md mx-auto mb-8 text-lg">Join over 5,000 curious readers getting thoughtful articles every week.</p>
        {submitted ? (
          <div className="inline-flex items-center gap-2 text-emerald-400 text-lg font-semibold"><Icon.CheckCircle size={22} /> You're subscribed — welcome aboard!</div>
        ) : (
          <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email address" required
              className="flex-1 bg-slate-800/80 border border-slate-700/50 rounded-xl px-5 py-3.5 text-slate-200 placeholder-slate-500 outline-none focus:border-sky-500/50 transition" />
